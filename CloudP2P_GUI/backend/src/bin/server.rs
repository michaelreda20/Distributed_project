...
const MAX_REQUEST_SIZE: u64 = 100 * 1024 * 1024; // 100MB max
...
fn configure_large_transfer_socket(stream: &TcpStream) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        use winapi::um::winsock2::{setsockopt, SOL_SOCKET, SO_SNDBUF, SO_RCVBUF, SOCKET};
        
        unsafe {
            let socket = stream.as_raw_socket() as SOCKET;
            let size: i32 = 8 * 1024 * 1024; // 8MB buffer
...
async fn handle_client_with_load_balancing(
    ...
    // Read metadata size with validation
    let meta_size = stream.read_u64().await?;
    info!("Received metadata size: {} bytes", meta_size);
    
    // CRITICAL: Validate metadata size
    if meta_size > MAX_REQUEST_SIZE {
        error!("INVALID metadata size: {} bytes (max: {})", meta_size, MAX_REQUEST_SIZE);
        let error_msg = format!("ERROR:Invalid metadata size: {} bytes", meta_size);
        let error_bytes = error_msg.as_bytes();
        stream.write_u64(error_bytes.len() as u64).await?;
        stream.write_all(error_bytes).await?;
        stream.flush().await?;
        bail!("Invalid metadata size: {}", meta_size);
    }
    ...
    info!("Read {} bytes of metadata", meta_buf.len());

    // Read image size with validation
    let img_size = stream.read_u64().await?;
    info!("Received image size: {} bytes ({:.2} MB)", img_size, img_size as f64 / 1_048_576.0);
    
    // CRITICAL: Validate image size
    if img_size > MAX_REQUEST_SIZE {
        error!("INVALID image size: {} bytes (max: {})", img_size, MAX_REQUEST_SIZE);
        let error_msg = format!("ERROR:Invalid image size: {} bytes", img_size);
        let error_bytes = error_msg.as_bytes();
        stream.write_u64(error_bytes.len() as u64).await?;
        stream.write_all(error_bytes).await?;
        stream.flush().await?;
        bail!("Invalid image size: {}", img_size);
    }
    ...
    info!("Read {} bytes of image data", img_buf.len());
    
    info!("Successfully received client request (meta: {} bytes, image: {} bytes)", 
          meta_size, img_size);
    ...
    let result = if best_server.server_id == raft_node.config.server_id {
        ...
        info!("Local processing completed in {}ms, result size: {} bytes", 
              elapsed, encrypted.len());
        encrypted
    } else {
        let target_address = best_addr
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("No address for server {}", best_server.server_id))?;
        
        info!("Forwarding to server {} at {}", best_server.server_id, target_address);
        
        let encrypted = forward_work_to_address(target_address, &meta_buf, &img_buf).await?;
        
        info!("Forwarded work completed, result size: {} bytes", encrypted.len());
        encrypted
    };

    // Send result back to client
    info!("Sending result to client: {} bytes", result.len());
    stream.write_u64(result.len() as u64).await?;
    stream.write_all(&result).await?;
    stream.flush().await?;
    
    info!("✓ Successfully sent result to client ({} bytes)", result.len());
    Ok(())
}
// =============================================================================
// HELPER FUNCTIONS
...
async fn process_encryption_work(meta_buf: &[u8], img_buf: &[u8]) -> Result<Vec<u8>> {
    ...
    tokio::task::spawn_blocking(move || {
        // Parse JSON from C++ client
        let json_value: serde_json::Value = serde_json::from_slice(&meta_buf)
            .map_err(|e| anyhow::anyhow!("Failed to parse JSON metadata: {}. Raw data: {}", 
                                         e, String::from_utf8_lossy(&meta_buf)))?;
        
        // Extract owner
        let owner = json_value["owner"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Missing 'owner' field in JSON"))?
            .to_string();
        
        // Extract quotas HashMap
        let mut quotas = HashMap::new();
        if let Some(quotas_obj) = json_value["quotas"].as_object() {
            for (key, value) in quotas_obj {
                if let Some(num) = value.as_u64() {
                    quotas.insert(key.clone(), num as u32);
                }
            }
        }
        
        // Create ImagePermissions
        let permissions = ImagePermissions { owner, quotas };
        
        info!("Parsed permissions: owner={}, quotas={:?}", 
              permissions.owner, permissions.quotas);
        ...
        let unified_image_bytes = fs::read("unified_image.png")
            .map_err(|e| anyhow::anyhow!("Failed to read unified_image.png: {}", e))?;
        ...
        info!("Successfully encrypted image: {} bytes", out_buf.len());
